...
import PostConsumeType from '../API/PostConsumeType'
import PutConsumeType from '../API/PutConsumeType'
import DeleteConsumeType from '../API/DeleteConsumeType'
import ValidationConsumeType from '../../../../src/Validation/Pharmacy/ValidationUnitMaster'
import GetConsumeType from '../API/GetConsumeType';
...
export default function ConsumeType() {

    let [consumeTypeData, setConsumeTypeData] = useState()
    ...
    let getdata = async () => {
        ...
        if (getResponse.status === 1) {
            // setLoder(0)
            setConsumeTypeData(getResponse.responseValue)
        }
    ...
    let handleDeleteRow = async () => {
        ...
        let response = await DeleteConsumeType(obj)
        if (response.status === 1) {
            setShowUnderProcess(0)
            setShowToster(1)
    ...
    let handleUpdate = async (id, consumeType, userId) => {

        setUpdateBool(1)
        setSendForm(sendForm => ({
            ...sendForm,
            "id": id,
            "manufacturer": consumeType,
            "userId": userId

        }))

        document.getElementById("consumeType").value = consumeType;
    ...
    let handleClear = () => {
        ...
        document.getElementById("consumeType").value = "";
        setUpdateBool(0)
    }
    useEffect(() => {
        getdata()
                ...
                <div className="container-fluid">
                    <div className="row">
                        <div className="col-12">
                            <Heading text='Consume Type Master' />
                            <BoxContainer>

                                <div className="mb-2 me-2">
                                    <label htmlFor="consumeType" className="form-label">Consume Type Name<span class="starMandatory">*</span></label>
                                    <input type='text' className="form-control form-control-sm" id='consumeType' name='consumeType' onChange={handleChange} placeholder='Enter consume type' />
                                </div>
                        ...
                        <div className="col-12 mt-2">
                            <Heading text=' Consume Type List' />
                            <div className="med-table-section" style={{ "height": "75vh" }}>
                                <TableContainer>
                                    <thead>
                                        <tr>
                                            <th className="text-center" style={{ "width": "5%" }}>#</th>
                                            <th>Consume Type Name</th>
                                    ...
                                    <tbody>
                                        {consumeTypeData && consumeTypeData.map((key, ind) => {
                                            return (
                                                <tr value={key.id}>
                                                    <td className="text-center">{ind + 1}</td>
                                                    <td>{key.consumeType}</td>

                                                    <td>
                                                        <div className="action-button">
                                                            <div data-bs-toggle="tooltip" data-bs-title="Edit Row" data-bs-placement="bottom"><img src={IconEdit} onClick={() => { handleUpdate(key.id, key.consumeType, key.userId) }} alt='' /></div>
