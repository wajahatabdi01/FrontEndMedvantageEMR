...
import GetRoleWiseMenuAssign from '../../Api/UtilityMaster/RoleWiseMenuAssign/GetRoleWiseMenuAssign'
import ValidationRoleWiseMenuAssign from '../../../Validation/SuperAdmin/UtilityMaster/ValidationRoleWiseMenuAssign'
import PostRoleWiseMenuAssign from '../../Api/UtilityMaster/RoleWiseMenuAssign/PostRoleWiseMenuAssign'
import DeleteRoleWiseMenuAssign from '../../Api/UtilityMaster/RoleWiseMenuAssign/DeleteRoleWiseMenuAssign'
import PutRoleWiseMenuAssign from '../../Api/UtilityMaster/RoleWiseMenuAssign/PutRoleWiseMenuAssign'
...
export default function RoleWiseMenuAssign() {
    let [menuList, setMenuList] = useState()
    let [roleList, setRoleList] = useState()
    let [headList, setHeadList] = useState()
    let [roleWiseMenuList, setroleWiseMenuList] = useState()
    ...
    //Handle Save
    let saveForm = async () => {
        let valresponse = ValidationRoleWiseMenuAssign(sendForm.roleId, sendForm.menuId, sendForm.headId)

        if (valresponse) {
            setShowUnderProcess(1)
            let response = await PostRoleWiseMenuAssign(sendForm);
    ...
    //Get data
    let getdata = async () => {
        let getResponse = await GetRoleWiseMenuAssign();
        ...
        if (getResponse.status === 1) {
            // setLoder(0)
            setroleWiseMenuList(getResponse.responseValue)
    ...
    let handleDeleteRow = async () => {
        // setLoder(1)
        setShowUnderProcess(1)
        let response = await DeleteRoleWiseMenuAssign(rowId)
        if (response.status === 1) {
            setShowUnderProcess(0)
            setShowToster(1)
    ...
    let handleUpdate = async (id, menuId, roleId, headId, superAdminUserId, roleTitle, menuName, headName) => {
        setUpdateBool(1)
        setSendForm(sendForm => ({
            ...sendForm,
            "id": id,
    ...
    let saveUpdate = async () => {
        ...
        if (valresponse) {
            setShowUnderProcess(1)
            let response = await PutRoleWiseMenuAssign(sendForm)
            if (response.status === 1) {
                setShowUnderProcess(0)
                setShowToster(1)
                ...
                <div className="container-fluid">
                    <div className="row">
                        <div className="col-12">
                            <Heading text='Role Wise Menu Assign' />
                            <BoxContainer>
                        ...
                        <div className="col-12 mt-2">
                            <div className='handlser'>
                                <Heading text="Role Wise Menu List" />
                            ...
                            <div className="med-table-section" style={{ "height": "75vh" }}>
                                <TableContainer>
                                    ...
                                    <tbody>
                                        {roleWiseMenuList && roleWiseMenuList.filter((val) => `${val.roleTitle} ${val.menuName} ${val.headName}`.toLowerCase().includes(searchTerm.toLowerCase())).map((val, ind) => {
                                            return (
                                                <tr key={val.id}>
                                                    ...
                                                    <td>
                                                        <div className="action-button">
                                                            <div data-bs-toggle="tooltip" data-bs-title="Edit Row" data-bs-placement="bottom"><img src={editBtnIcon} className='' alt='' onClick={() => { handleUpdate(val.id, val.menuId, val.roleId, val.headId, val.superAdminUserId, val.roleTitle, val.menuName, val.headName) }} /></div>
