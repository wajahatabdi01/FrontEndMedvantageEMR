...
import ValidationRoleMaster from '../../../Validation/SuperAdmin/UtilityMaster/ValidationRoleMaster'
import PostRoleMaster from '../../Api/UtilityMaster/RoleMaster/PostRoleMaster'
import GetRoleMaster from '../../Api/UtilityMaster/RoleMaster/GetRoleMaster'
import DeleteRoleMaster from '../../Api/UtilityMaster/RoleMaster/DeleteRoleMaster'
import PutRoleMaster from '../../Api/UtilityMaster/RoleMaster/PutRoleMaster'
...
export default function RoleMaster() {
  ...
  //Handle Save
  let saveForm = async () => {
    let valresponse = ValidationRoleMaster(sendForm.roleTitle, sendForm.code)

    if (valresponse) {
      setShowUnderProcess(1)
      let response = await PostRoleMaster(sendForm);
  ...
  //Get data
  let getdata = async () => {
    let getResponse = await GetRoleMaster();
    if (getResponse.status === 1) {
      setRoleList(getResponse.responseValue)
    }
  ...
  let handleDeleteRow = async () => {
    // setLoder(1)
    setShowUnderProcess(1)
    let response = await DeleteRoleMaster(rowId)
    if (response.status === 1) {
      setShowUnderProcess(0)
      setShowToster(1)
  ...
  let handleUpdate = async (id, roleTitle, code, superAdminUserId) => {
    setUpdateBool(1)
    setSendForm(sendForm => ({
      ...sendForm,
      "id": id,
  ...
  let saveUpdate = async () => {
    ...
    if (valresponse) {
      setShowUnderProcess(1)
      let response = await PutRoleMaster(sendForm)
      if (response.status === 1) {
        setShowUnderProcess(0)
        setShowToster(1)
        ...
        <div className="container-fluid">
          <div className="row">
            <div className="col-12">
              <Heading text='Role Master' />
              <BoxContainer>
                <div className="mb-2 me-2">
                  <label htmlFor="roleTitle" className="form-label">Role Name <span className="starMandatory">*</span></label>
                  <input type="text" className="form-control form-control-sm" id="roleTitle" name='roleTitle' onChange={handleChange} placeholder="Enter Role Name" />
                </div>
                <div className="mb-2 me-2">
            ...
            <div className="col-12 mt-3">
              ...
              <div className="med-table-section" style={{ "height": "73vh" }}>
                <TableContainer>
                  ...
                  <tbody>
                    {roleList && roleList.filter((val) => `${val.roleTitle} ${val.code}`.toLowerCase().includes(searchTerm.toLowerCase())).map((val, ind) => {
                      return (
                        <tr key={val.id}>
                          ...
                          <td>
                            <div className="action-button">
                              <div data-bs-toggle="tooltip" data-bs-title="Edit Row" data-bs-placement="bottom"><img src={editBtnIcon} className='' alt='' onClick={() => { handleUpdate(val.id, val.roleTitle, val.code, val.superAdminUserId) }} /></div>
