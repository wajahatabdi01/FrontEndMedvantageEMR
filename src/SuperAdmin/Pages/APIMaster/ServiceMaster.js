...
import ValidationServiceMaster from '../../../Validation/SuperAdmin/APIMaster/ValidationServiceMaster'
import PostServiceMaster from '../../Api/APIMaster/ServiceMaster/PostServiceMaster'
import GetServiceMaster from '../../Api/APIMaster/ServiceMaster/GetServiceMaster'
import DeleteServiceMaster from '../../Api/APIMaster/ServiceMaster/DeleteServiceMaster'
import PutServiceMaster from '../../Api/APIMaster/ServiceMaster/PutServiceMaster'
...
export default function ServiceMaster() {
  ...
  //Handle Save
  let saveForm = async () => {
    let valresponse = ValidationServiceMaster(sendForm.name)
    if (valresponse) {
      setShowUnderProcess(1)
      let response = await PostServiceMaster(sendForm);
  ...
  //Get data
  let getdata = async () => {
    let getResponse = await GetServiceMaster();
    if (getResponse.status === 1) {
      // setLoder(0)
      setServiceName(getResponse.responseValue)
  ...
  let handleDeleteRow = async () => {
    // setLoder(1)
    setShowUnderProcess(1)
    let response = await DeleteServiceMaster(rowId)
    if (response.status === 1) {
      setShowUnderProcess(0)
      setShowToster(1)
  ...
  let saveUpdate = async () => {
    ...
    if (valresponse) {
      setShowUnderProcess(1)
      let response = await PutServiceMaster(sendForm)
      if (response.status === 1) {
        setShowUnderProcess(0)
        setShowToster(1)
        ...
        <div className="container-fluid">
          <div className="row">
            <div className="col-12">
              <Heading text='Service Master' />
              <BoxContainer>
                <div className="mb-2 me-2">
                  <label htmlFor="name" className="form-label">Service Name <span className="starMandatory">*</span></label>
                  <input type="text" className="form-control form-control-sm" id="name" name="name" onChange={handleChange} placeholder="Enter service name" />
                </div>
            ...
            <div className="col-12 mt-2">
              ...
              <div className="med-table-section" style={{ "height": "73vh" }}>
                <TableContainer>
                  ...
                  <tbody>
                    {serviceName && serviceName.filter((val) => `${val.name}`.toLowerCase().includes(searchTerm.toLowerCase())).map((val, ind) => {
                      return (
                        <tr key={val.id}>
