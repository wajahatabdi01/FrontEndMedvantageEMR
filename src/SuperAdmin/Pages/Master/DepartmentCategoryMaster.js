...
import ValidationDepartmentCategoryMaster from "../../../Validation/SuperAdmin/Master/ValidationDepartmentCategoryMaster"
import PostDepartmentCategoryMaster from "../../Api/Master/DepartmentCategoryMaster/PostDepartmentCategoryMaster"
import GetDepartmentCategoryMaster from "../../Api/Master/DepartmentCategoryMaster/GetDepartmentCategoryMaster"
import DeleteDepartmentCategoryMaster from "../../Api/Master/DepartmentCategoryMaster/DeleteDepartmentCategoryMaster"
import PutDepartmentCategoryMaster from "../../Api/Master/DepartmentCategoryMaster/PutDepartmentCategoryMaster"

export default function DepartmentCategoryMaster() {
    ...
    //Handle Save
    let saveForm = async () => {
        let valresponse = ValidationDepartmentCategoryMaster(sendForm.categoryName)

        if (valresponse) {
            // setLoder(1)
            setShowUnderProcess(1)
            let response = await PostDepartmentCategoryMaster(sendForm);
    ...
    //Get data
    let getdata = async () => {
        let getResponse = await GetDepartmentCategoryMaster();
    ...
    let handleDeleteRow = async () => {
        // setLoder(1)
        setShowUnderProcess(1)
        let response = await DeleteDepartmentCategoryMaster(rowId)
        if (response.status === 1) {
            setShowUnderProcess(0)
            setShowToster(1)
    ...
    let saveUpdate = async () => {
        ...
        if (valresponse) {
            setShowUnderProcess(1)
            let response = await PutDepartmentCategoryMaster(sendForm)
            if (response.status === 1) {
                setShowUnderProcess(0)
                setShowToster(1)
                ...
                <div className="container-fluid">
                    <div className="row">
                        <div className="col-12">
                            <Heading text='Department Category Master' />
                            <BoxContainer>
                        ...
                        <div className="col-12 mt-2">
                            <div className='handlser'>
                                <Heading text="All Department Category List" />
                            ...
                            <div className="med-table-section" style={{ "height": "74vh" }}>
                                <TableContainer>
                                    ...
                                    <tbody>
                                        {categoryList && categoryList.filter((val) => `${val.categoryName}`.toLowerCase().includes(searchTerm.toLowerCase())).map((val, ind) => {
                                            return (
                                                <tr key={val.id}>
                                                    ...
                                                    <td>
                                                        <div className="action-button">
                                                            <div data-bs-toggle="tooltip" data-bs-title="Edit Row" data-bs-placement="bottom"><img src={editBtnIcon} className='' alt='' onClick={() => { handleUpdate(val.id, val.categoryName, val.superAdminUserId) }} /></div>
