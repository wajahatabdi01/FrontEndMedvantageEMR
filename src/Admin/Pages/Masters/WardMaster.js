...
export default function WardMaster() {
    ...
    //Handle Save
    let saveForm = async () => {
        let valresponse = ValidationWardMaster(sendForm.wardName)

        if (valresponse) {
            setShowUnderProcess(1)
            let response = await PostAPIWardMaster(sendForm);
    ...
    let getdata = async () => {
        ...
        let getResponse = await GetAPIWardMaster(clientID);
    ...
    let handleDeleteRow = async () => {
        ...
        let response = await DeleteAPIWardMaster(obj)
        if (response.status === 1) {
            setShowUnderProcess(0)
            setShowToster(1)
    ...
    let handleUpdate = async (id, wardName, userId) => {
        setUpdateBool(1)
        setSendForm(sendForm => ({
            ...sendForm,
            "id": id,
    ...
    let saveUpdate = async () => {
        ...
        if (valresponse) {
            setShowUnderProcess(1)
            let response = await PutAPIWardMaster(sendForm)
            if (response.status === 1) {
                setShowUnderProcess(0)
                setShowToster(1)
                ...
                <div className="container-fluid">
                    <div className="row">
                        <div className="col-12">
                            <Heading text={t("Ward_Master")} />
                            <BoxContainer>
                                <div className="mb-2 me-2">
                                    <label htmlFor="wardName" className="form-label">{t("Ward_Nm")}<span className="starMandatory">*</span></label>
                                    <input type="text" className="form-control form-control-sm" id="wardName" name='wardName' onChange={handleChange} placeholder={t("Ward_Name")} />
                                </div>
                                                ...
                                                <tr key={val.id}>
                                                    ...
                                                    <td>
                                                        ...
                                                        <div className="action-button">
                                                            <div data-bs-toggle="tooltip" data-bs-title="Edit Row" data-bs-placement="bottom"><img src={IconEdit} onClick={() => { handleUpdate(val.id, val.wardName, val.userId) }} alt='' /></div>
