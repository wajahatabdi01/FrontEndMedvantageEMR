...
import GetFHIRReferralSourceMaster from '../Api/FHIRReferralSourceMaster/GetFHIRReferralSourceMaster';
import PostFHIRReferralSourceMaster from '../Api/FHIRReferralSourceMaster/PostFHIRReferralSourceMaster';
import PutFHIRReferralSourceMaster from '../Api/FHIRReferralSourceMaster/PutFHIRReferralSourceMaster';
import DeleteFHIRReferralSourceMaster from '../Api/FHIRReferralSourceMaster/DeleteFHIRReferralSourceMaster';



export default function FHIRReferralSourceMaster() {
    
  let [ReferralSource,setReferralSource]= useState("")
  let [ReferralSourceListMain,setReferralSourceListMain] = useState("");
  ...
  let GetReferralSource = async()=>{
     const response = await GetFHIRReferralSourceMaster()
      if(response.status === 1){
        setReferralSource(response.responseValue);
        setReferralSourceListMain(response.responseValue);
  ...
  //handle search
  let handleSearch = (e) => {
    let resp = Search(ReferralSourceListMain, e.target.value)
    if (e.target !== "") {
      if (resp.length !== 0) {
        setReferralSource(resp)
      }
      else {
        setReferralSource([])
      }
        ...
        setReferralSource(ReferralSourceListMain)
    }
  }
  ...
  const handleSave = async () => {
    if (sendForm.name === '' || sendForm.name === null || sendForm.name === undefined) {
      document.getElementById('errName').innerHTML = "Issue name is required";
      document.getElementById('errName').style.display = "block";
    }
    else {
      setShowUnderProcess(1);

      const response = await PostFHIRReferralSourceMaster({
        ...sendForm,
      });
      if (response.status === 1) {
  ...
  const handlerUpdate = async () => {
    if (sendForm.name === '' || sendForm.name === null || sendForm.name === undefined) {
      ...
      setShowUnderProcess(1);
      const response = await PutFHIRReferralSourceMaster({
        ...sendForm,
      });
      if (response.status === 1) {
        setShowUnderProcess(0);
  ...
  const handleDelete = async () => {
    ...
    const response = await DeleteFHIRReferralSourceMaster(obj);
    if (response.status === 1) {
      setShowLoder(0)
      setisShowToaster(1);
    ...
    <div className="container-fluid">
        <div className="row">
            <div className="col-12">
              <Heading text="ReferralSource Master"/>
                <BoxContainer>
                   <div className="col-2 mb-2 me-2">
                       <label htmlFor="name" className="form-label">ReferralSource Name<span className="starMandatory">*</span></label>
            ...
            <div className="col-12 mt-2">
                <div className="handlser">
                    <Heading text="ReferralSource List"/>
                    ...
                    <TableContainer>
                        <thead>
                            <tr>
                                ...
                                <th>ReferralSource Name</th>
                                <th>Code</th>
                                <th className="text-center" style={{"width": "10%"}}>Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            {ReferralSource && ReferralSource.map((val,ind)=>{
                                return(
                                    <tr key={val.id}>
