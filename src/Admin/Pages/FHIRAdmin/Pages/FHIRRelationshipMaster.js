...
import GetFHIRRelationshipMaster from '../Api/FHIRRelationshipMaster/GetFHIRRelationshipMaster';
import PostFHIRRelationshipMaster from '../Api/FHIRRelationshipMaster/PostFHIRRelationshipMaster';
import PutFHIRRelationshipMaster from '../Api/FHIRRelationshipMaster/PutFHIRRelationshipMaster';
import DeleteFHIRRelationshipMaster from '../Api/FHIRRelationshipMaster/DeleteFHIRRelationshipMaster';



export default function FHIRRelationshipMaster() {
    
  let [relationshipList,setRelationshipList]= useState("")
  let [relationshipMain,setRelationshipMain] = useState("");
  ...
  let GetRelationshipList = async()=>{
   
     const response = await GetFHIRRelationshipMaster()
      if(response.status === 1){
        setRelationshipList(response.responseValue);
        setRelationshipMain(response.responseValue);
  ...
  //handle search
  let handleSearch = (e) => {
    let resp = Search(relationshipMain, e.target.value)
    if (e.target !== "") {
      if (resp.length !== 0) {
      ...
      setRelationshipList(relationshipMain)
    }
  }
  ...
  const handleSave = async () => {
    if (sendForm.name === '' || sendForm.name === null || sendForm.name === undefined) {
      ...
      const response = await PostFHIRRelationshipMaster({
        ...sendForm,
      });
      if (response.status === 1) {
  ...
  const handlerUpdate = async () => {
    if (sendForm.name === '' || sendForm.name === null || sendForm.name === undefined) {
      ...
      setShowUnderProcess(1);
      const response = await PutFHIRRelationshipMaster({
        ...sendForm,
      });
      if (response.status === 1) {
        setShowUnderProcess(0);
  ...
  const handleDelete = async () => {
    ...
    const response = await DeleteFHIRRelationshipMaster(obj);
    if (response.status === 1) {
      setShowLoder(0)
      setisShowToaster(1);
    ...
    <div className="container-fluid">
        <div className="row">
            <div className="col-12">
              <Heading text="Relationship Master"/>
                <BoxContainer>
                   <div className="col-2 mb-2 me-2">
                       <label htmlFor="name" className="form-label">Relationship Name<span className="starMandatory">*</span></label>
            ...
            <div className="col-12 mt-2">
                <div className="handlser">
                    <Heading text="Relationship Master List"/>
                ...
                <div className="med-table-section" style={{"height" : "75vh"}}>
                    <TableContainer>
                        ...
                        <tbody>
                            {relationshipList && relationshipList.map((val,ind)=>{
                                return(
                                    <tr key={val.id}>
