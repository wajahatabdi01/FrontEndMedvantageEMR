...
import GetFHIRTypeMaster from '../Api/FHIRTypeMaster/GetFHIRTypeMaster';
import PostFHIRTypeMaster from '../Api/FHIRTypeMaster/PostFHIRTypeMaster';
import PutFHIRTypeMaster from '../Api/FHIRTypeMaster/PutFHIRTypeMaster';
import DeleteFHIRTypeMaster from '../Api/FHIRTypeMaster/DeleteFHIRTypeMaster';

export default function FHIRTypeMaster() {
  ...
  const getData = async () => {
    setShowLoder(1);
    const response = await GetFHIRTypeMaster();
  ...
  const handlerSave = async () => {
    if (sendForm.name === '' || sendForm.name === null || sendForm.name === undefined) {
      ...
      const response = await PostFHIRTypeMaster({
        ...sendForm,
      });
      if (response.status === 1) {
  ...
  const handlerUpdate = async () => {
    if (sendForm.name === '' || sendForm.name === null || sendForm.name === undefined) {
      ...
      setShowUnderProcess(1);
      const response = await PutFHIRTypeMaster({
        ...sendForm,
      });
      if (response.status === 1) {
        setShowUnderProcess(0);
  ...
  const handleDelete = async () => {
    ...
    const response = await DeleteFHIRTypeMaster(obj);
    if (response.status === 1) {
      setShowLoder(0)
      setisShowToaster(1);
