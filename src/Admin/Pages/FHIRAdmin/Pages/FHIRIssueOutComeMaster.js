...
import GetIssueOutComeMaster from '../Api/IssueOutComeMaster/GetIssueOutComeMaster';
import PostIssueOutComeMaster from '../Api/IssueOutComeMaster/PostIssueOutComeMaster';
import PutIssueOutComeMaster from '../Api/IssueOutComeMaster/PutIssueOutComeMaster';
import DeleteIssueOutComeMaster from '../Api/IssueOutComeMaster/DeleteIssueOutComeMaster';


export default function FHIRIssueOutComeMaster() {
    
  let [issueOutCome,setIssueOutCome]= useState("")
  let [outComeListMain,setOutComeListMain] = useState("");
  ...
  let GetIssueList = async()=>{
     ...
     const response = await GetIssueOutComeMaster()
      if(response.status === 1){
        setIssueOutCome(response.responseValue);
        setOutComeListMain(response.responseValue);
  ...
  //handle search
  let handleSearch = (e) => {
    let resp = Search(outComeListMain, e.target.value)
    if (e.target !== "") {
      if (resp.length !== 0) {
        setIssueOutCome(resp)
      }
      else {
        setIssueOutCome([])
      }
      ...
      setIssueOutCome(outComeListMain)
    }
  }
  ...
  const handleSave = async () => {
    if (sendForm.name === '' || sendForm.name === null || sendForm.name === undefined) {
      ...
      const response = await PostIssueOutComeMaster({
        ...sendForm,
      });
      if (response.status === 1) {
  ...
  const handlerUpdate = async () => {
    if (sendForm.name === '' || sendForm.name === null || sendForm.name === undefined) {
      ...
      setShowUnderProcess(1);
      const response = await PutIssueOutComeMaster({
        ...sendForm,
      });
      if (response.status === 1) {
        setShowUnderProcess(0);
  ...
  const handleDelete = async () => {
    ...
    const response = await DeleteIssueOutComeMaster(obj);
    if (response.status === 1) {
      setShowLoder(0)
      setisShowToaster(1);
    ...
    <div className="container-fluid">
        <div className="row">
            <div className="col-12">
              <Heading text="IssueOutCome Master"/>
                <BoxContainer>
                   <div className="col-2 mb-2 me-2">
                       <label htmlFor="name" className="form-label">Issue Name<span className="starMandatory">*</span></label>
            ...
            <div className="col-12 mt-2">
                <div className="handlser">
                    <Heading text="Issue Master List"/>
                ...
                <div className="med-table-section" style={{"height" : "75vh"}}>
                    <TableContainer>
                        ...
                        <tbody>
                            {issueOutCome && issueOutCome.map((val,ind)=>{
                                return(
                                    <tr key={val.id}>
